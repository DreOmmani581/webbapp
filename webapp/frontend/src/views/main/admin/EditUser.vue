        ...
        <template>
          ...
          <v-form v-model='valid' ref='form' lazy-validation>
            ...
            <div class='subheading secondary--text text--lighten-2'>
              ...
              ><span v-else>(currently not a teacher)</span>
            </div>
            <v-checkbox label='Is a Teacher' v-model='isTeacher'></v-checkbox>
...
export default class EditUser extends Vue {
  ...
  public reset() {
    ...
    if (this.user) {
      ...
      this.isTeacher = this.user.is_teacher;
    }
  }
  ...
  public async del() {
    if (await this.$validator.validateAll()) {
      ...
      await dispatchUpdateDelete(this.$store, {
        id: this.user!.id,
        user: updatedProfile,
      });
