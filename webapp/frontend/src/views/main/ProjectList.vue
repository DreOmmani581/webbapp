<template>
  <div id='app'>
    <header>
      ...
      <label for='company'
        >Suche nach einem Projektnamen oder einer Gruppe</label
      >
      <input
        type='text'
        ...
        v-model='company'
        name='company'
        v-on:keydown.enter.prevent='getPlaceholder'
      />
      <button @click='getPlaceholder'>Suchen</button>
    </header>
    <div class='user-options'>
      <span @click='toggleListView()'
        ><font-awesome-icon
          icon='fa-solid fa-list'
          :class="isList ? 'list-icon-color' : ''" /><font-awesome-icon
          icon='fa-solid fa-border-all'
          :class="isList ? '' : 'list-icon-color'"
      /></span>
      <label for='sort-options' id='sort-options-label'
        >Sort by:
        <select name='sort-options' id='sort-options' v-model='sortOn'>
          <option value='starCount'>Meiste Bewertungen</option>
...
export default {
    ...
    sortedArray() {
      ...
      if (this.sortOn === 'name' || this.sortOn === 'language') {
        return arrayToSort.sort((a, b) => {
          ...
          const aFormatted =
            a[this.sortOn] !== null ? a[this.sortOn].toLowerCase() : null;
          const bFormatted =
            b[this.sortOn] !== null ? b[this.sortOn].toLowerCase() : null;
        ...
        return arrayToSort.sort((a, b) =>
          a[this.sortOn] > b[this.sortOn] ? -1 : 1,
        );
      }
    },
    morePagesAvailable() {
  ...
  methods: {
    ...
    async getRepos(company) {
      ...
      await axios
        .get(`${baseGithubApi}/${company}/${githubApiParams}`)
        .then((res) =>
    ...
    async paginateRepos() {
      ...
      await axios
        .get(
          `${baseGithubApi}/${this.currentCompany}/${githubApiParams}&page=${this.page}`,
        )
        .then((res) =>
          res.data.forEach((repo) => {
            this.repoInfo.push({
    ...
    async getCommits(company, name) {
      this.commitsLoaded = false;
      await axios
        .get(`https://api.github.com/repos/${company}/${name}/commits`)
        .then((res) => {
          res.data.forEach((commit) => {
            ...
            commit.author !== null
              ? (commitInfo.username = commit.author.login)
              : (commitInfo.username = commit.commit.author.email);
